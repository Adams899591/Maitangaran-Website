    ...
    <div class="bg-white rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.08)] border border-gray-100 p-6 md:p-8">
      ...
      <form wire:submit.prevent="processCheckout" class="space-y-6">
        ...
        <div class="mt-8 pt-6 border-t border-gray-100 flex items-center justify-between gap-4">
          <!-- Back to Address (Update with your actual route name) -->
          <a 
            href="{{route("delivery-method")}}" 
            class="px-6 py-3.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold text-sm rounded-lg transition-colors cursor-pointer text-center" wire:navigate
          >
            &larr; Back
          </a>
          ...
          <button 
            ...
            @if(empty($shippingRates)) disabled @endif
            class="px-8 py-3.5 bg-black hover:bg-gray-800 disabled:bg-gray-300 text-white font-bold text-sm rounded-lg transition-colors cursor-pointer shadow-md flex items-center justify-center gap-2 whitespace-nowrap"
          >
